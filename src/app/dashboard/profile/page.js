...
import CandidatePortfolioSection, {
  CANDIDATE_ENTRY_CONFIG,
  normalizeCandidateCollections,
} from "@/components/profile/CandidatePortfolioSection";
...
export default function ProfilePage() {
  ...
  const handleAddCandidateEntry = async (entryType, entryDraft) => {
    if (!entryType || !entryDraft || !user?.id || !profile) {
      return;
    }
    ...
    const config = CANDIDATE_ENTRY_CONFIG[entryType];
    ...
    try {
      ...
      console.error(`Failed to save ${entryType}:`, saveErr);
      setError(
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {isCandidateRole(profile?.role) && (
          <CandidatePortfolioSection
            ...
            subtitle="Add education, certificates, and experience directly from your profile."
            onAddEntry={handleAddCandidateEntry}
          />
        )}
