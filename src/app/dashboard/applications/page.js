...
export default function ApplicationsPage() {
  ...
  async function loadJobSeekerApplications(userId) {
    try {
      ...
      const enrichedApplications = applicationsData.map((application) => {
        const job =
          jobsData?.find((entry) => entry.id === application.job_id) || {};
        const orgProfile =
          organizationProfiles.find(
            (entry) => entry.id === job.organization_id,
          ) || {};
  ...
  async function loadEmployerApplications(employerId) {
    try {
      ...
      const enrichedApplications = applicationsData.map((application) => {
        const job =
          employerJobs.find((entry) => entry.id === application.job_id) || {};
        const applicantProfile =
          applicantProfiles?.find(
            (entry) => entry.id === application.applicant_id,
          ) || {};
                  ...
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <div className="mt-2">
                        <button
                          type="button"
                          className="text-sm text-[#FF1E00] hover:underline"
                        ...
                        >
                          {app.cover_letter && (
                            <div>
                              ...
                              <p className="text-gray-600 text-sm p-3 bg-gray-50 rounded-lg whitespace-pre-wrap">
                                {app.cover_letter}
                              </p>
                            </div>
                          )}
                          ...
                          {isJobSeeker && app.job?.description && (
                            <div>
                              <h4 className="text-sm font-medium text-gray-700 mb-2">
                                Job Description:
                              </h4>
                          ...
                          {isEmployer && app.applicant_profile?.bio && (
                            <div>
                              <h4 className="text-sm font-medium text-gray-700 mb-2">
                                Applicant Bio:
                              </h4>
